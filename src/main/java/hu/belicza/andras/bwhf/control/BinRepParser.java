    ...
    private static Action readNextAction(final int frame, final ByteBuffer commandsBuffer, final int commandBlocksEndPos, final GameChatWrapper gameChatWrapper) {
        ...
        switch (blockId) {
            ...
            case (byte) 0x15: { // Attack/Right Click/Cast Magic/Use ability
                ...
                final short posY = commandsBuffer.getShort();
				/*final short unitId = */
                commandsBuffer.getShort(); // (posX;posY) if this is 0xffff, or target this unit if it's a valid unit id (if it's not 0xffff)
                ...
                switch (type) {
                    ...
                    case (byte) 0x0e: // Attack move
                        ...
                        break;
                    case (byte) 0x28:
                        actionNameIndex = Action.ACTION_NAME_INDEX_SET_RALLY;
                        break;
                    default:
                        actionNameIndex = Action.ACTION_NAME_INDEX_UNKNOWN;
                        break;
                }
